...
- **4 GNSS-созвездия, 12 верифицированных сигналов** — одновременная мультисистемная генерация в один IQ-файл
...
| `gps_bds_gal_l1_300s.json` | GPS+BDS+GAL L1 (над-эфир, 52 dBHz) | 8 MHz |
| `quad_l1g1.json` | GPS+BDS+GAL+GLO L1/G1 | 46.5 MHz |

В репозитории 46 пресетов. Большинство используют Montana location, длительность 10s, elevation mask 5°, полный RINEX; пресеты `moscow_*` и `usa_*` — длительные сценарии для реальных приёмников (60–300s) в соответствующих локациях.
...
### Воспроизведение по эфиру (HackRF / PortaPack → реальный приёмник)

При проигрывании сгенерированного файла по эфиру в реальный приёмник (телефон, u-blox) дефолты, настроенные под софт-верификатор, слишком слабы. Чтобы телефон **захватил и удержал трекинг** (декодировал эфемериды → получил fix):

- **`initPower` ≥ 52 dBHz** в пресете. AGC нормирует файл к фиксированному выходному RMS независимо от `initPower`, поэтому он задаёт внутрифайловый CN0 (запас сигнал/шум), а не громкость. Дефолтные 45 dBHz захватываются верификатором, но телефон не удержит трекинг.
- **Частота дискретизации ≥ 8 МГц.** HackRF One умеет 2–20 Msps, но `<8 МГц` вне спецификации DAC; replay на PortaPack ограничен скоростью SD, поэтому ~8–10 Msps — практический оптимум.
- **E1/B1C ниже 12.276 МГц автоматически используют чистый BOC(1,1).** Настоящая поднесущая CBOC/QMBOC BOC(6,1) (6.138 МГц) при низкой частоте дискретизации алиасится в канал данных и блокирует декод nav-сообщения на реальных приёмниках; генератор откатывается на чистый BOC(1,1) (который узкий L1-фронтенд телефона и так использует) и включает настоящий CBOC только при `Fs ≥ 12.276 МГц`.
- **BeiDou на телефонах — это B1I (1561.098 МГц), а не B1C (1575.42 МГц).** B1I вне полосы L1-файла, поэтому для BeiDou на телефоне нужен отдельный B1I-пресет.

`presets/gps_bds_gal_l1_300s.json` уже настроен под это (8 МГц, 52 dBHz, 300 с).
...
**Single-system после июньских проходов 2026 (честные CBOC/QMBOC + NH-устойчивый верификатор):**

| Сигнал | Sample Rate | RINEX Visible | Detected | z-score Range |
|--------|-------------|--------------|----------|---------------|
| Galileo E1 (CBOC) | 5 MHz | 10 | 10/10 | 63–103 |
| BeiDou B1C (QMBOC, акквизиция по пилоту) | 5 MHz | 12 | 12/12 | 132–247 |
...
> PRN-коды сверены бит-в-бит с независимыми эталонами: B1I — gnss-sdr (63/63 PRN); B1C data/pilot/secondary, E5a I/Q, B2a data/pilot и все NH/вторичные коды — PocketSDR. GPS L5 XA/XB у нас ICD-буквальный (свободный регистр XB; mod-10230 свёртка эталонов расходится в последних `adv` чипах, эффект <0.12 дБ).
>
> Верификатор использует линейную корреляцию двойным окном (устойчив к флипам NH/вторичных чипов на эпохах кода) с компенсацией кодового допплера — прежний результат 0/10 по GPS L5 был артефактом верификатора, что подтверждено чистым дампом с идеальной корреляцией каждого периода и бит-точной NH-разводкой.
