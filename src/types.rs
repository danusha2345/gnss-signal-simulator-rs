...
pub struct GlonassTime {
    /// N4 — индекс 4-летнего цикла от эпохи 1996-01-01 (0..7+).
    /// NOT a leap-year flag, несмотря на историческое имя поля.
    /// Заполняется как `years/4` в `gnsstime.rs::utc_to_glonass_time_corrected`.
    pub LeapYear: i32,
    /// Кумулятивный день от эпохи 1996-01-01: `N4·1461 + day_in_cycle + 1`.
    /// Это не GLONASS NA (день в 4-летнем цикле, 1..1461) — внутренний счётчик.
    pub Day: i32,
    /// Миллисекунды от начала дня (0..86_399_999) с применённым +3h Moscow offset.
    pub MilliSeconds: i32,
